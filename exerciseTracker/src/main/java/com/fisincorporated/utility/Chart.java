...
public class Chart {
	 ...
	 */
	public GraphicalView createBarChart(Context context, String chartTitle,
			String xAxisTitle, String yAxisTitle, String[] valueTitles,
			double minX, double maxX, double minY, double maxY,
			ArrayList<double[]> values, int[] colors, String[] xLabels,
		...
		// width is in pixels so need method to set according to screen resolution
		// renderer.setBarSpacing(1.5f);
		if (values.get(0).length < 12) {
			renderer.setXAxisMax(values.get(0).length * values.size());
		...
		// disable default xLabels and add the passed in values
