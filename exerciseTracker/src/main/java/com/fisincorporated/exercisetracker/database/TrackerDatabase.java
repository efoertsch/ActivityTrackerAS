...
public final class TrackerDatabase {
	...
	public static final class ProgramSetting implements BaseColumns {
		private ProgramSetting() {
		}

		public static final String PROGRAM_SETTING_TABLE = "program_setting";
		public static final String NAME = "name";
		public static final String VALUE = "value";
		public static final String DEFAULT_SORT_ORDER = "value ASC";
	}
	public final static String getCreateProgramSettingTableSQL() {
		// Note BaseColumns defines _ID
		return "CREATE TABLE IF NOT EXISTS  " + ProgramSetting.PROGRAM_SETTING_TABLE
				+ "  (" + ProgramSetting._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT ," 
				+ ProgramSetting.NAME 	+ " TEXT NOT NULL COLLATE NOCASE, " 
				+ ProgramSetting.VALUE + " TEXT NOT NULL "
				+  ");";
	}
	
	//return program setting based on name of program setting key
	public final static String getProgramSettingValueSQL(String key){
		return "select value from " + ProgramSetting.PROGRAM_SETTING_TABLE 
				+ " where name = '" +  key + "'"  ;
	}
	...
	public static final class Exercise implements BaseColumns {
		...
		public static final String[] exerciseColumnNames = {
			...
			,ELEVATION_IN_DIST_CALCS,  MIN_DISTANCE_TO_LOG};
	} 
	...
	/*
	 * public final static String getCreateTriggerInsertActivity(){ return
	 * "CREATE TRIGGER fk_insert_activity " + " BEFORE INSERT ON " +
	 * Exercise.EXERCISE_TABLE +
	 * " FOR EACH ROW BEGIN  SELECT RAISE(ROLLBACK, 'insert on table \" " +
	 * Exercise.EXERCISE_TABLE +
	 * "\" violates foreign key constraint \"fk_activity\"')" +
	 * " WHERE  (SELECT _ID FROM " + ExrcsLocation.LOCATION_TABLE +
	 * "  WHERE _ID = NEW." + LocationExercise.LOCATION_ID + ") IS NULL; END;";
	 * }
	 */
	...
	/*
	 * public final static String getCreateTriggerInsertGPSLog(){ return
	 * " Create TRIGGER fk_insert_gpsLoger " + " BEFORE INSERT ON " +
	 * GPSLog.GPSLOG_TABLE +
	 * " FOR EACH ROW BEGIN  SELECT RAISE(ROLLBACK, 'insert on table \" " +
	 * GPSLog.GPSLOG_TABLE +
	 * "\" violates foreign key constraint \"fk_activityid\"')" +
	 * " WHERE  (SELECT _ID FROM " + Exercise.EXERCISE_TABLE +
	 * "  WHERE _ID = NEW." + GPSLog.EXERCISE_LOCATION_ID + ") IS NULL; END;"; }
	 */
